...
function memoize(func) {
  ...
  return (args) => {
    ...
    cache.set(key, func.call(this, key));
    return cache.get(key);
  };
}
...
function logger(func, logFunc) {
  return (...args) => {
    const argumentsList = args.map((arg) => JSON.stringify(arg)).join(',');
    logFunc.call(null, `${func.name}(${argumentsList}) starts`);
    const result = func.apply(null, [...args]);
    logFunc.call(null, `${func.name}(${argumentsList}) ends`);
    return result;
  };
}
